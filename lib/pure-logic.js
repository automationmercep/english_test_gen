...
// Polish plural for "pytanie" (question): 1 → pytanie, 2–4 → pytania, else pytań
// (with the usual 12–14 exception). Returns the noun only; prefix the count.
...
// Return the indexes of all answers that use a grid cell. A classic crossword
// can have two (across + down); quizcross/keycross cells normally have one.
function crosswordEntryIndexesAtCell(entries, row, col) {
  return (Array.isArray(entries) ? entries : []).reduce((indexes, entry, index) => {
    if (Array.isArray(entry.cells) && entry.cells.some(cell => cell[0] === row && cell[1] === col)) indexes.push(index);
    return indexes;
  }, []);
}

// Keep the direction already being typed when possible. For a freshly clicked
// crossing, prefer the horizontal answer, which matches common crossword UX.
function chooseCrosswordEntryIndex(entries, row, col, preferredIndex = -1) {
  const indexes = crosswordEntryIndexesAtCell(entries, row, col);
  if (indexes.includes(preferredIndex)) return preferredIndex;
  const across = indexes.find(index => entries[index].dir === "across");
  return across === undefined ? (indexes[0] ?? -1) : across;
}

// Find the cell that should receive focus after typing (step 1) or pressing
// Backspace in an empty cell (step -1). Movement follows the active answer's
// cells, never the row-major order of input elements in the DOM.
function crosswordFocusTarget(entries, row, col, preferredIndex = -1, step = 1) {
  const direction = step < 0 ? -1 : 1;
  const entryIndex = chooseCrosswordEntryIndex(entries, row, col, preferredIndex);
  if (entryIndex < 0) return null;
  const entry = entries[entryIndex];
  const cellIndex = entry.cells.findIndex(cell => cell[0] === row && cell[1] === col);
  const adjacentCell = entry.cells[cellIndex + direction];
  if (adjacentCell) return { entryIndex, row: adjacentCell[0], col: adjacentCell[1] };

  const adjacentEntryIndex = entryIndex + direction;
  const adjacentEntry = entries[adjacentEntryIndex];
  if (!adjacentEntry || !adjacentEntry.cells.length) return null;
  const targetCell = direction > 0 ? adjacentEntry.cells[0] : adjacentEntry.cells[adjacentEntry.cells.length - 1];
  return { entryIndex: adjacentEntryIndex, row: targetCell[0], col: targetCell[1] };
}
...
// ── Cloze (gap-fill) question type ("Uzupełnij luki") ───────────────────────
// The author pastes a text and wraps each word/phrase to hide in square
// brackets, e.g. "I [go] to school and [play] football". parseClozeSegments
// splits it into an ordered list of segments — plain text runs and gaps — so the
// player renderer can interleave text with input fields. Escaped "\[" / "\]"
// (or doubled "[[" "]]") are treated as literal brackets, not gap markers.
// clozeGapAnswers returns just the answers in order (used for scoring/counting).
function parseClozeSegments(text) {
  const src = String(text || "");
  const segments = [];
  let buffer = "";
  let i = 0;
  const pushText = () => { if (buffer) { segments.push({ type: "text", value: buffer }); buffer = ""; } };
  while (i < src.length) {
    const ch = src[i];
    // Literal bracket via backslash escape (\[ or \]) or doubled ([[ or ]]).
    if (ch === "\\" && (src[i + 1] === "[" || src[i + 1] === "]")) { buffer += src[i + 1]; i += 2; continue; }
    if (ch === "[" && src[i + 1] === "[") { buffer += "["; i += 2; continue; }
    if (ch === "]" && src[i + 1] === "]") { buffer += "]"; i += 2; continue; }
    if (ch === "[") {
      const close = src.indexOf("]", i + 1);
      if (close === -1) { buffer += ch; i++; continue; } // unmatched "[" → literal
      const answer = src.slice(i + 1, close).trim();
      if (answer) { pushText(); segments.push({ type: "gap", answer }); }
      else buffer += src.slice(i, close + 1); // empty [] → keep literally
      i = close + 1;
      continue;
    }
    buffer += ch;
...
function clozeGapAnswers(text) {
  return parseClozeSegments(text).filter(s => s.type === "gap").map(s => s.answer);
}

// ── Bulk type conversion: data-shape "families" ─────────────────────────────
// The "change all questions to type X" action moves each card's content across
// types. That is lossless only WITHIN a family whose data shape matches; across
// families it drops content (e.g. a crossword's clue list has nowhere to go in a
// single-answer "fill" question). Grouping types into families lets the UI warn
// before a conversion silently discards work.
//  - text:      prompt + one answer  (choice, fill, order, flashcard, anagram, correct)
//  - crossword: prompt + clue list   (crossword, quizcross, keycross)
//  - match:     prompt + pair list   (match)
//  - wordsearch:prompt + word list   (wordsearch)
const QUESTION_TYPE_FAMILIES = {
  choice: "text", fill: "text", order: "text", flashcard: "text", anagram: "text", correct: "text",
  crossword: "crossword", quizcross: "crossword", keycross: "crossword",
  match: "match",
  wordsearch: "wordsearch",
  cloze: "cloze",
};
function questionTypeFamily(type) {
  return QUESTION_TYPE_FAMILIES[type] || "text";
}
// True when converting `fromType` → `toType` may discard content because the two
// types store fundamentally different data shapes (different families). Same
// family (or same type) is always considered safe.
function isLossyTypeConversion(fromType, toType) {
  if (fromType === toType) return false;
  return questionTypeFamily(fromType) !== questionTypeFamily(toType);
}
