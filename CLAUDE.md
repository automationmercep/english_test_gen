...
- `.github/workflows/tests.yml` runs the Playwright suite on every push/PR to `main` and uploads its HTML report (including videos and failure screenshots) as a GitHub Actions artifact retained for 14 days. It does not commit generated reports to `main`, so the branch can remain protected.
...
A quiz is `{ id, title, level, category, image, shuffleQuestions, shuffleAnswers, questions[] }`. Each question has a `type` — `choice`, `fill`, `order`, `match`, `flashcard`, `correct`, `anagram`, `wordsearch`, `crossword`, `quizcross`, `keycross`, or `cloze` — and type-specific fields (e.g. `answers`/`correctAnswers` for `choice`, `pairs` for `match`, `answer`/`words` for `order`, `wrong`/`answer` for `correct`, `answer` for `anagram`, `words[]` for `wordsearch`, `clues[]` of `{answer, clue}` for `crossword`/`quizcross`/`keycross`, plus a `key` string for `keycross`, and a `text` string for `cloze`). An optional `instruction` field, if set, overrides the default type-label badge shown above the question during play (e.g. "Complete the sentence" instead of "Wybierz odpowiedź").
...
- **`cloze`** — a gap-fill: the question stores a `text` string where words to hide are wrapped in square brackets, e.g. `I [go] to school and [play] football`. `parseClozeSegments(text)` (pure-logic) splits it into ordered `text`/`gap` segments; `clozeGapAnswers(text)` returns the answers in order. `renderClozeAnswer` interleaves the text with inline `<input>`s whose width tracks each answer's length (`max(4, len+2)` ch) so the word fits. `selectedAnswer` is an array of strings (one per gap); correct when every gap matches its answer via `matchesTextAnswer` (case-insensitive). `\[`/`\]`/`[[`/`]]` are literal brackets; empty `[]` and unmatched `[` are treated as plain text. CSV row is `prompt, text with [gaps], cloze` (quote the text if it contains commas); synonyms `cloze` / `luki` / `uzupelnij luki` / `gap fill`. It is its own bulk-conversion family, so switching to/from it warns about content loss.
...
- **Bulk type change** ("Typ wszystkich pytań" bar): `setAllQuestionTypes(type)` runs `switchQuestionType` on every card at once. The buttons are grouped in the UI by data-shape *family* — text (choice/fill/order/flashcard/anagram/correct), crossword (crossword/quizcross/keycross), match, wordsearch — because switching *within* a family round-trips content losslessly but switching *across* families drops the answer data (only the `prompt` survives). Before any cross-family switch that would discard non-empty content, `setAllQuestionTypes` shows a `confirm()` naming how many questions are affected (empty cards don't count). `questionTypeFamily` / `isLossyTypeConversion` (in `lib/pure-logic.js`) encode the family map and are unit-tested; `cardHasAnswerContent` (in `app.js`) checks whether a live card holds droppable content.
